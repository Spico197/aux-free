...
def expert_load(
    ...
    cat_gate_logits = torch.stack(gate_logits, dim=0)
    routing_weights = torch.nn.functional.softmax(cat_gate_logits, dim=-1)
    _, selected_experts = torch.topk(routing_weights, top_k, dim=-1)
    # (layers, tokens, topk, experts)
    expert_mask = torch.nn.functional.one_hot(selected_experts, num_experts)
    n_layers, n_tokens, n_topk, n_experts = expert_mask.shape

    if attention_mask is not None:
        # (tokens,)
        attn_mask = attention_mask.view(-1)
        # (1, tokens, 1, 1)
        attn_mask = attn_mask[None, :, None, None]
        expert_mask = expert_mask.float() * attn_mask.float()

    expert_mask = expert_mask.reshape(n_layers, n_tokens * n_topk, n_experts)
    # (layers, experts)
    expert_load = torch.sum(expert_mask.float(), dim=1)
    return expert_load


def maxvio(expert_load):
    # (layers, experts)
    max_violation = torch.max(expert_load, dim=-1, keepdim=True).values
    avg_load = torch.mean(expert_load, dim=-1, keepdim=True)
    return (max_violation - avg_load) / avg_load
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # NOTE(tzhu): use the original scores instead of the biased ones
        routing_weights = scores.gather(-1, selected_experts)

        # NOTE(tzhu): normalize the routing weights (beaware of the eps)
        denominator = routing_weights.sum(dim=-1, keepdim=True) + 1e-20
        routing_weights /= denominator
...
class MixtralForCausalLM(MixtralPreTrainedModel, GenerationMixin):
    ...
    def update_bias(self, expert_load):
        if self.config.balance_type != "noaux_tc":
            return

        # expert_load: (layers, experts)
        n_layers, _ = expert_load.shape
        for layer_idx in range(n_layers):
            curr_load = expert_load[layer_idx]
