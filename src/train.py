...
from src.modeling.configuration_mixtral import MixtralConfig
from src.modeling.modeling_mixtral import (
    MixtralForCausalLM,
    expert_load,
    maxvio,
)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="The name of the tokenizer to use (via the tokenizers library).",
    )
    parser.add_argument(
        "--use_slow_tokenizer",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default="noaux_tc",
        help="Balance type.",
        choices=["noaux_tc", "gshard"],
    )
    parser.add_argument(
        "--update_rate",
        type=float,
        default=0.001,
        help="Update rate for aux-free biases.",
    )
...
def main():
    ...
    config.balance_type = args.balance_type
    ...
    if args.with_tracking:
        ...
        lr_scheduler_type_value = experiment_config["lr_scheduler_type"].value
        experiment_config["lr_scheduler_type"] = lr_scheduler_type_value
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                ...
                loss = outputs.loss
                expert_load_val = expert_load(
                    outputs.router_logits,
                    num_experts=config.num_local_experts,
                    top_k=config.num_experts_per_tok,
                    attention_mask=batch["attention_mask"],
                )
                maxvio_val = maxvio(expert_load_val)
                maxvio_val = maxvio_val.mean()
